...
psql "$DATABASE_URL" -f migrations/004_sync.sql
```
...
`accounts` (incl. `plan`), `customers`, `checkout_sessions`, `credit_ledger`,
`cli_tokens`, `synced_usage`, `synced_invoices`, `synced_snapshots`.
